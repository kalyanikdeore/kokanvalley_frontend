  ...
  return (
    <div className="relative w-full overflow-hidden">
      ...
      >
        ...
        {/* Content Container */}
        <div className="relative z-10 container mx-auto px-4 md:px-6" style={{ marginBottom: isMobile ? "-100px" : "-148px" }}>
          <div className="max-w-2xl" style={{ marginLeft: isMobile ? '20px' : '60px' }}>
            ...
            <motion.p
              className="text-lg md:text-xl lg:text-2xl text-green-50 mb-6 md:mb-8 max-w-lg"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.3 }}
            >
            ...
            >
              <a 
                ...
                className="bg-white text-green-700 hover:bg-green-100 px-4 py-2 md:px-6 md:py-3 rounded-full font-medium flex items-center justify-center transition-all text-sm md:text-base"
              >
                <Play className="w-4 h-4 md:w-5 md:h-5 mr-2" />
                Watch Video
              </a>
              <button className="bg-green-700 text-white hover:bg-green-800 px-4 py-2 md:px-6 md:py-3 rounded-full font-medium flex items-center justify-center transition-all text-sm md:text-base">
        ...
        <button
          ...
          className="absolute top-8 right-4 md:right-8 bg-green-700/80 hover:bg-green-800 p-2 md:p-3 rounded-full backdrop-blur-sm transition-all z-10"
          aria-label={isFavorite ? "Remove from favorites" : "Add to favorites"}
        >
          <Heart
            className={`w-5 h-5 md:w-6 md:h-6 ${isFavorite ? "fill-white text-white" : "text-white"}`}
          />
        </button>
      </div>
      ...
      {heroContent.length > 1 && (
        <div className="container mx-auto px-4 md:px-6" style={{ marginTop: isMobile ? "-80px" : "-16px" }}>
          <div className="bg-green-50/90 backdrop-blur-sm rounded-xl p-3 md:p-4 shadow-2xl max-w-4xl mx-auto border border-green-100">
            <div className="flex justify-center gap-2 md:gap-4 overflow-x-auto py-1">
              {heroContent.map((item, index) => (
                <button
                  key={item.id}
                  onClick={() => selectImage(index)}
                  className={`flex-shrink-0 relative cursor-pointer rounded-lg overflow-hidden transition-all ${isMobile ? 'w-20 h-12' : 'w-32 h-20'} ${
                    currentIndex === index
                ...
                >
                  ...
                  >
                    {currentIndex === index && <Play className="text-white w-3 h-3 md:w-5 md:h-5" />}
                  </div>
                </button>
              ))}
            </div>
